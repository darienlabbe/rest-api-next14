...
export const PATCH = async (request: Request, context: {params: any}) => {
    ...
    try {
        ...
        if (!categoryId ||!Types.ObjectId.isValid(categoryId)) {
            return new NextResponse(JSON.stringify({ message: "Invalid or missing category" }), { status: 400 });
        }
...
export const DELETE = async (request: Request, context: {params: any}) => {
    ...
    try {
        ...
        return new NextResponse(JSON.stringify({message: "Category deleted", categoryId}), {status: 200});
